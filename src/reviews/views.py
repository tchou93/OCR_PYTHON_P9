...
def posts(request):
    tag = "posts"
    user_all_tickets = (Ticket.objects.filter(user=request.user)).annotate(content_type=Value('TICKET', CharField()))
    user_all_reviews = (Review.objects.filter(user=request.user)).annotate(content_type=Value('REVIEW', CharField()))
    ...
    if request.method == 'POST':
        ...
        if r_form.is_valid() and t_form.is_valid():
            new_ticket = Ticket.objects.create(title=request.POST['title'],
                                               ...
                                               user=request.user,
                                               image=image)
            Review.objects.create(ticket=new_ticket,
